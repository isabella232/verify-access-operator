...
image_name = "verify-access-operator-build"
...
This script can be used to manage a docker Verify Access operator build 
...
3. You need a copy of the verify-access-operator repository that you are 
   building.  
...
'create': Used to create a new container for a specific workspace.  When the 
          ...
          container will stop once you exit from the shell.  For example:
            'operator_build.py create --container build.operator \
                --source /Users/dev/github/verify-access-operator'
...
def process_argv():
    ...
    parser = argparse.ArgumentParser(description="This script is designed "
        "to help manage the verify access operator Docker build container.  At "
    ...
    cmd = subparsers.add_parser("build", help="This command will " 
        "construct the verify access operator build image.")
    ...
    cmd.add_argument("-s", "--source", help="The name of the directory "
        "which contains the verify-access-operator source code.  Default: "
        "current path",
        required=False)
